# -*- coding: utf-8 -*-
"""Project2_ecoli.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/19bx1O2tZsyV2J97JQFVPeFVKlGllGoaj
"""
...
# Ecoli Type Classification KEEL Dataset (~84%)
eco_df = pd.read_csv('Ecoli2.csv',names=["Mcg","Gvh","Lip","Chg","Aac","Alm1","Alm2","Class"],index_col=False)
...
x = ["Mcg","Gvh","Lip","Chg","Aac","Alm1","Alm2"]
y = ["Class"]

y_eco = eco_df[y]
x_eco = eco_df[x]
x_train, x_test, y_train, y_test = train_test_split(x_eco, y_eco, test_size=0.3)

# Desicion Tree Classifier
dt_eco = DecisionTreeClassifier().fit(x_train,y_train)
dt_scores = cross_val_score(dt_eco,x_test,y_test,cv=10)

# Random Forest Classifier
rf_eco = RandomForestClassifier().fit(x_train,y_train)
rf_scores = cross_val_score(rf_eco,x_test,y_test,cv=10)
...
for train, test in skf.split(x_eco, y_eco):
  skf_xtrain, skf_xtest = x_eco.loc[train,:"Alm2"], x_eco.loc[test,:"Alm2"] 
  skf_ytrain, skf_ytest = y_eco.loc[train,:"Class"], y_eco.loc[test,:"Class"]
...
x_cluster.insert(0, x_cluster.pop(9))
...
for i in range(0,9):
  ...
  plt.savefig('Elbow_Ecoli.png', dpi=300)
...
for i in range(0,9):
  ...
  plt.savefig('Silhouette_Ecoli.png', dpi=300)
...
plt.savefig('Metrics_Ecoli.png', dpi=300)
plt.show()